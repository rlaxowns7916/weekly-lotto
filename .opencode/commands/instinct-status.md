...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/skills/continuous-learning-v2/.opencode/plugins/cache/everything-claude-code/scripts/instinct-cli.py" status
```

Or if `CLAUDE_PLUGIN_ROOT` is not set (manual installation), use:

```bash
python3 ~/.claude/skills/continuous-learning-v2/.opencode/plugins/cache/everything-claude-code/scripts/instinct-cli.py status
```
