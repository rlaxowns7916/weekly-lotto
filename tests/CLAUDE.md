...
- SCN-002: Given 모바일 구매 페이지 진입 또는 셀렉터 표시 실패, When 대기 조건이 타임아웃, Then `testStatus=failed` and `diagnosticAttachmentCount>=1`.

## 오류 계약
- 에러 코드: `E2E_PAGE_NOT_READY`(페이지 진입 실패), `E2E_SELECTOR_TIMEOUT`(요소 대기 실패).
