...
## 실패 상세 진단 구현 정책
- 테스트 실패 diagnostics는 운영 코드와 매핑 가능한 `error.code` 후보를 포함하도록 유지한다.
- `waitVisibleWithReason` 경유 실패에서 `context/url/maintenance/selectors` 필드를 기반으로 `NETWORK|DOM|PARSE` 분류 검증을 수행한다.
- 운영 코드가 `UNKNOWN_UNCLASSIFIED`를 반환하는 경우 테스트는 `classificationReason` 존재를 검증한다.
- OCR/HTML 기능 추가 후 실패 경로에서 아티팩트 수집과 첨부 정책(10MB 상한, 부분 첨부)을 회귀 검증한다.

## 관측성
- 실패 시 Playwright screenshot/video/trace를 저장한다.
- diagnostics attachment(`*-diagnostics`)를 텍스트로 기록한다.
- OCR/HTML 아티팩트 검증 결과를 테스트 로그 또는 attachment로 기록한다.
