 ...
 *
 * 실행:
 *   npm run pension:check-result                    # 최근 회차 당첨 확인
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // 2. 당첨 번호 조회
    console.log('\n2. 당첨 번호 조회 중...');
    const winningNumbers = await fetchLatestPensionWinning(session.page);

    if (!winningNumbers) {
      console.log('\n' + '='.repeat(50));
      console.log('⏳ 당첨 번호를 조회할 수 없습니다.');
      console.log('='.repeat(50));
      console.log('\n   연금복권 추첨은 매주 목요일 저녁에 진행됩니다.');
    ...
    console.log(`   ${winningNumbers.round}회 당첨 번호: ${winningNumbers.winningGroup}조 ${winningNumbers.winningNumber} / 보너스 ${winningNumbers.bonusNumber}`);
