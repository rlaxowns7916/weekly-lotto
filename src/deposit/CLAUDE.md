...
- 포함: 충전 전/후 예치금 잔액 대조를 통한 충전 발생 여부 판정 제공.
...
- 유효성 규칙:
  ...
  - 실충전 성공 판정은 완료 다이얼로그가 아니라 충전 전/후 예치금 잔액 대조를 1순위 근거로 사용한다.
- 출력 타입/필드:
  - 충전 결과(`ChargeResult`: amount, status, timestamp, keypadOcrConfidence, balance?, verification?, dialogConfirmed?).
...
- SCN-004: Given 충전 제출 후 완료 다이얼로그를 확인하지 못했으나 잔액이 충전 금액 이상 증가, When 충전 명령을 실행, Then `processExitCode=0` and `status='success'` and 충전 제출 1회.
- SCN-005: Given 충전 제출 후 결과를 판정할 수 없음(잔액 조회 실패), When 충전 명령을 실행, Then `processExitCode=1` and `error.code='DEPOSIT_VERIFICATION_FAILED'` and 충전 제출 1회(재시도 없음).
...
- 재시도 가능 여부: 조건부. 충전 제출 이후에는 잔액 대조로 충전 미발생이 확인된 경우에만 재시도한다.
- 발생 조건: 로그인 실패, 충전 페이지 이동 실패, 키패드 OCR 인식 실패, 비밀번호 입력 실패, 충전 미발생, 충전 결과 판정 불가, 이메일 전송 실패.

## 불변식/제약
- 트랜잭션 경계: 없음.
- 정합성 규칙: 각 명령은 브라우저 세션 생성 후 `finally`에서 종료를 시도한다.
- 멱등성 규칙: `DRY_RUN=true` 경로는 충전 상태를 변경하지 않는다.
- 멱등성 규칙: 충전 제출 재시도는 잔액이 변하지 않은 것으로 확인된 경우로 한정해 중복 충전을 구조적으로 차단한다.
- 순서 보장 규칙: 로그인 선행 후 충전 페이지 이동 → 금액 선택 → 키패드 인식 → 충전 전 잔액 조회 → 비밀번호 입력 → 완료 대기 → 충전 후 잔액 조회 → 판정 순으로 수행한다.
...
- [ ] 충전 전/후 잔액을 수집해 충전 발생 여부를 판정하고 결과에 포함한다.
- [ ] 완료 다이얼로그 타임아웃이 곧바로 실패로 보고되지 않는다.
- [ ] 판정 불가 상태에서 충전을 재시도하지 않는다.
