...
## 기능 범위/비범위
- 포함: 충전 페이지 URL/금액/버튼/키패드/다이얼로그/잔액 셀렉터 상수 제공.
- 포함: 충전 실행, 예치금 잔액 조회, 키패드 OCR 인식, 비밀번호 입력 액션 제공.
...
- 유효성 규칙:
  ...
  - `balanceCandidates`의 1순위는 실측 확인된 `#tooltipTotalAmt`(`.list-charge-wrap .txt-amount` 내부)이며, 모두 빗나가면 본문 텍스트 스캔으로 폴백한다.
- 출력 타입/필드:
  - `depositSelectors` 상수(`balanceCandidates` 포함).
  - `ChargeResult`.
  - `KeypadDigitMap`.
...
- 순서 보장 규칙: 페이지 이동 → 금액 선택 → 충전 버튼 → 키패드 OCR → 충전 전 잔액 조회 → 비밀번호 입력 → 충전 후 잔액 조회 순서를 따른다.
...
- [ ] 잔액 조회가 후보 셀렉터 실패 시 본문 텍스트 스캔으로 폴백한다.
