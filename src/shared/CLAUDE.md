...
## 기능 범위/비범위
- 포함: 브라우저 세션 생성/종료 및 오류 스크린샷 저장 기능 제공.
...
## 공개 인터페이스 계약
- 입력 타입/필드:
  - `process.env` 설정값, Playwright `Page`, 이메일 옵션(`subject`, `html`, `text`).
...
- SCN-002: Given 설정 누락 또는 브라우저/SMTP 오류, When shared API가 실패를 처리, Then `errorThrown=true` or `result.success=false`.

## 오류 계약
- 에러 코드: 고정 에러 코드 상수는 없다(예외/실패 결과 메시지 기반 처리).
...
- 타임아웃: 주요 이동 60초, 요소 대기 10~30초, 스크린샷 저장 5초 제한 정책을 따른다.
- 동시성 요구: 공유 전역 상태는 `getConfig()` 캐시에 한정되며, 그 외 유틸은 함수 호출 단위로 동작한다.

## 의존성 계약
- 내부 경계: `src/shared/browser`, `src/shared/browser/actions`, `src/shared/config`, `src/shared/services`, `src/shared/utils`.
