...
- 스크린샷 저장:
  ...
  - 실패 시 `null` 반환.
- HTML 스냅샷 저장:
  - `artifacts/html-failures/{name}-{timestamp}-main.html` 및 `...-frame-{i}.html` 규칙으로 저장.
  - 실패 시 `html.status=FAILED`와 `html.failureReason` 반환.
...
- HTML 캡처 실패는 주 실패 원인을 덮어쓰지 않고 상태 객체로 반환한다.

## 관측성
- 스크린샷 저장 성공/실패를 콘솔에 출력.
- HTML 스냅샷 저장 성공/실패 및 frame 개수를 콘솔에 출력.
- 별도 메트릭/트레이싱 구현은 없다.
